...
including transforms in the lists of `JOB_ROUTER_PRE_ROUTE_TRANSFORMS` and `JOB_ROUTER_PRE_ROUTE_TRANSFORMS`,
respectively.
...
`JOB_ROUTER_TRANSFORM_<name>` to the lists of `JOB_ROUTER_PRE_ROUTE_TRANSFORMS` and `JOB_ROUTER_POST_ROUTE_TRANSFORMS`,
respectively.
...
JOB_ROUTER_PRE_ROUTE_TRANSFORMS = $(JOB_ROUTER_PRE_ROUTE_TRANSFORMS) Periodic_Hold
```

To apply the same transform after your pre-route and route transforms, append the name of the transform to
`JOB_ROUTER_POST_ROUTE_TRANSFORMS` instead:

```
JOB_ROUTER_POST_ROUTE_TRANSFORMS = $(JOB_ROUTER_POST_ROUTE_TRANSFORMS) Periodic_Hold
```
