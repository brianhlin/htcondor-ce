...
1.  Each transform in `JOB_ROUTER_PRE_ROUTE_TRANSFORMS` whose requirements are met by the job
...
-   **If you are using the [ClassAd transform syntax](#classad-transforms),** transforms in
    `JOB_ROUTER_PRE_ROUTE_TRANSFORMS` and `JOB_ROUTER_PRE_ROUTE_TRANSFORMS` may also have their own requirements that
    determine whether or not that transform is applied.
