...
COLL_MAPFILE_FILES	:= config/condor_mapfile.central_collector
...
_mkdirs:
	...
	mkdir -p $(DESTDIR)/$(INSTALL_SHARE_DIR)/condor-ce/{ceview-plugins,config.d,plugins,templates}
	...
	mkdir -p $(DESTDIR)/$(INSTALL_SYSCONF_DIR)/condor-ce/{config.d,metrics.d,passwords.d,webapp.tokens.d}
...
_view: _mkdirs
	...
	install -p -m 0644 -D -t $(DESTDIR)/$(INSTALL_SHARE_DIR)/condor-ce/			$(VIEW_SHARE_FILES)
...
# these boys special
# /etc/condor-ce/config.d/05-ce-view-table.conf  
# 	config/05-ce-view-table-defaults.conf
...
collector: client _view
	...
	install -p -m 0644 -D -t $(DESTDIR)/$(INSTALL_SYSCONF_DIR)/condor-ce/		$(COLL_MAPFILE_FILES)
